...
function SubmitCounts(props) {
  ...
  const [initialSubmitters, setInitialSubmitters] = useState([
    props.submitters,
  ]);
  ...
  function handleSearch(event) {
    ...
    if (event.target.searchInput.value !== "") {
      ...
      for (let i = 0; i < initialSubmitters.length; i++) {
        if (
          initialSubmitters[i].firstName.includes(searchString) ||
          initialSubmitters[i].lastName.includes(searchString) ||
          initialSubmitters[i].desc.includes(searchString)
        ) {
  ...
  function removeSubmitter(subId) {
    deleteSubmitters(subId).then(
      getSubmitters().then((_submitters) => setInitialSubmitters(_submitters))
    );
  }
    ...
    <React.Fragment>
      ...
      {showFromList
        ? listToShow.map((list) => (
            <div
              className="profile"
              key={list.firstName + list.lastName}
              id={list.id}
            >
              <SubmitterFrame>
        ...
        : initialSubmitters.map((mitter) => (
            <SubmitterFrame key={mitter.firstName + mitter.lastName}>
              <div className="profile" id={mitter.id}>
                <SearchResult
                  result={mitter}
                  removeSubmitter={removeSubmitter}
                />
              </div>
            </SubmitterFrame>
          ))}
