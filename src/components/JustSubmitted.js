...
function JustSubmitted(props) {
      ...
      if (subLen === i + 1) {
        result = (
          <div className="profile">
            {" "}
            <h3>Last one to submit</h3>
            <Info>
              <div>
                <div className="subFirstName">{sub.firstName}</div>{" "}
                {sub.lastName}
                <span> submitted {sub.time}</span>.
              </div>
              <div className="description">
                <span>Description:</span> {sub.desc}
              </div>
            </Info>
          </div>
        );
    ...
    return subsList.map((sub) => (
      <div className="profile" key={sub.firstName + sub.lastName}>
        <Info>
          <div>
            {" "}
            <div className="subFirstName">{sub.firstName}</div> {sub.lastName}
            <span> submitted {sub.time}</span>.
          </div>
          <div className="description">
            <span>Description: </span> {sub.desc}
  ...
  return (
    <>
      {submitter != null ? (
        <FirstSubmitterFrame>{submitter}</FirstSubmitterFrame>
      ) : (
        <div></div>
      )}
